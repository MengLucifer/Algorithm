...
class SequenceVerifier:
    ...
    Single frame could be coincidence; 10 in order is near-impossible to fake.
    """

    def __init__(self, sequence_len: int = 10, max_gap_sec: float = 2.0):
        self.sequence_len = sequence_len
        self.max_gap      = max_gap_sec
        self.buffer       = []   # [(video_name, video_sec, hamming_dist)]
...
class SceneCutTimeline:
    ...
    Gap pattern = time between consecutive cuts.
    This is unique per video and survives re-encoding.
    Example: cuts at [4.2, 18.7, 45.1] → gaps [14.5, 26.4]
    """

    CUT_THRESHOLD = 30    # mean pixel diff to call it a scene cut
    MIN_CUT_GAP   = 2.0   # seconds — ignore rapid flicker cuts

    def __init__(self, db_path: str):
        self.db_path  = db_path
        self.cut_db   = {}   # {video_name: [cut_sec, ...]}
        self.gap_db   = {}   # {video_name: [gap_sec, ...]}

        # Runtime state
        self.live_cuts      = []    # detected cut timestamps (real time)
        self.live_gaps      = []    # gaps between live cuts
        self.last_gray      = None
...
class MotionPatternVerifier:
    ...
    Creates a unique "motion signature" per video that survives re-encoding
    because motion depends on content, not pixel values.
    """
    ...
    def __init__(self, db_path: str):
        self.db_path   = db_path
        self.motion_db = {}   # {video_name: [energy_float, ...]}
...
class SpatialZoneVerifier:
    ...
    Creates a spatio-temporal signature unique per video.
    Brightness survives re-encoding better than exact pixel values.
    """
    ...
    def __init__(self, db_path: str):
        self.db_path = db_path
        self.zone_db = {}   # {video_name: [[zone_values], ...]}
...
# ===========================================================================
# VERIFICATION — TIER 2c: CYCLE-AWARE TIMESTAMP TRACKER
# ===========================================================================

class CycleAwareVerifier:
    """
    TIER 2 — Step 6
    Handles 15-hour live streams cycling through 1.5hr and 2hr videos.
    Detects:
      - Which video is currently playing
      - When a new cycle/video starts (timestamp rewind or name change)
      - Whether progression is natural within each cycle
    """

    def __init__(self, tolerance_sec: float = 3.0, min_chain: int = 5):
        self.tolerance   = tolerance_sec
        self.min_chain   = min_chain

        self.current_video     = None
        self.cycle_start_real  = None
        self.last_vt           = None
        self.last_rt           = None
        self.progression_ok    = 0
        self.progression_bad   = 0
        self.cycle_count       = {}
        self.completed_cycles  = []

    def add_match(self, video_name: str, video_sec: float,
                  real_time: float) -> dict:
        new_video = (video_name != self.current_video)
        rewound   = (self.last_vt is not None and
                     video_sec < self.last_vt - 10)

        if new_video or rewound:
            self._close_cycle(real_time)
            self._start_cycle(video_name, video_sec, real_time)

        status = self._check_progression(video_sec, real_time)
        self.last_vt = video_sec
        self.last_rt = real_time

        return {
            "status":           status,
            "current_video":    self.current_video,
            "current_vt":       video_sec,
            "cycle_count":      dict(self.cycle_count),
            "total_cycles":     sum(self.cycle_count.values()),
            "completed_cycles": self.completed_cycles[-3:],
        }

    def _check_progression(self, video_sec: float,
                           real_time: float) -> str:
        if self.last_vt is None or self.last_rt is None:
            return "UNCERTAIN"

        real_elapsed  = real_time - self.last_rt
        video_elapsed = video_sec - self.last_vt

        if video_elapsed < 0:
            return "REWOUND"

        drift = abs(video_elapsed - real_elapsed)
        if drift <= self.tolerance and video_elapsed >= 0:
            self.progression_ok += 1
        else:
            self.progression_bad += 1

        total = self.progression_ok + self.progression_bad
        if total < self.min_chain:
            return "UNCERTAIN"

        ratio = self.progression_ok / total
        return "VERIFIED" if ratio >= 0.70 else "MISMATCH"

    def _start_cycle(self, video_name: str, video_sec: float,
                     real_time: float):
        self.current_video    = video_name
        self.cycle_start_real = real_time
        self.last_vt          = None
        self.last_rt          = None
        self.progression_ok   = 0
        self.progression_bad  = 0
        self.cycle_count[video_name] = \
            self.cycle_count.get(video_name, 0) + 1
        logging.info("[CYCLE] New cycle: %s (play #%d)",
                     video_name, self.cycle_count[video_name])

    def _close_cycle(self, real_time: float):
        if self.current_video is None:
            return
        duration = (real_time - self.cycle_start_real
                    if self.cycle_start_real else 0)
        self.completed_cycles.append({
            "video":    self.current_video,
            "duration": round(duration, 1),
            "play_no":  self.cycle_count.get(self.current_video, 1),
        })
        logging.info("[CYCLE] Closed: %s played %.0fs",
                     self.current_video, duration)


# ===========================================================================
# VERIFICATION — TIER 3: NORMALIZED SHA-256
# ===========================================================================

class NormalizedSHAVerifier:
    """
    TIER 3 — Step 7 (supporting hint only)
    Normalizes frames to 32×32 grayscale with 16 brightness levels,
    then computes SHA-256. Absorbs small encoding differences.
    Treat as supporting evidence — heavy re-encoding may still cause mismatch.
    """

    NORM_SIZE       = (32, 32)
    QUANTIZE_LEVELS = 16
    SEGMENT_SIZE    = 10   # frames per segment SHA

    def __init__(self, db_path: str):
        self.db_path  = db_path
        self.frame_db = {}   # {video_name: {str(sec): sha256}}

        # Runtime
        self.live_buffer = []   # (matched_sec, normalized_bytes, video_name)

    def _normalize(self, frame: np.ndarray) -> bytes:
        small = cv2.resize(frame, self.NORM_SIZE,
                           interpolation=cv2.INTER_AREA)
        gray  = cv2.cvtColor(small, cv2.COLOR_BGR2GRAY) \
                if len(small.shape) == 3 else small
        step      = 256 // self.QUANTIZE_LEVELS
        quantized = (gray // step).astype(np.uint8)
        blurred   = cv2.GaussianBlur(quantized, (3, 3), 0)
    ...
    # ── Build ────────────────────────────────────────────────
    def build(self, video_dir: str, sample_fps: int = 1):
        exts  = {".mp4", ".mkv", ".avi"}
        files = [f for f in Path(video_dir).rglob("*")
                 if f.suffix.lower() in exts]

        for vf in files:
            logging.info("[SHA] Building normalized SHA db: %s", vf.name)
            ...
            while True:
                ...
                if idx % interval == 0:
                    sec        = round(idx / fps, 2)
                    shas[str(sec)] = self._sha(self._normalize(frame))
                idx += 1

            cap.release()
            self.frame_db[vf.name] = shas
            logging.info("[SHA]   %d frame SHAs stored", len(shas))

        Path(self.db_path).parent.mkdir(parents=True, exist_ok=True)
        with open(self.db_path, "w") as f:
            json.dump(self.frame_db, f)
        logging.info("[SHA] DB saved: %s", self.db_path)

    def load(self):
        with open(self.db_path) as f:
            self.frame_db = json.load(f)
        logging.info("[SHA] Loaded %d video SHA profiles", len(self.frame_db))

    # ── Runtime — single frame ───────────────────────────────
    def verify_frame(self, live_frame: np.ndarray,
                     matched_video: str, matched_sec: float) -> dict:
        live_sha   = self._sha(self._normalize(live_frame))
        video_shas = self.frame_db.get(matched_video, {})

        for db_sec_str, db_sha in video_shas.items():
            if abs(float(db_sec_str) - matched_sec) <= 2.0:
                if live_sha == db_sha:
                    return {"status":  "SHA_VERIFIED",
                            "video":   matched_video,
                            "at_sec":  matched_sec,
                            "sha":     live_sha[:16] + "..."}

        return {"status":   "SHA_MISMATCH",
                "note":     "May be re-encoding difference — use as hint only"}

    # ── Runtime — segment (stronger) ────────────────────────
    def verify_segment(self, live_frame: np.ndarray,
                       matched_video: str, matched_sec: float) -> dict:
        norm = self._normalize(live_frame)
        self.live_buffer.append((matched_sec, norm, matched_video))

        if len(self.live_buffer) > self.SEGMENT_SIZE * 2:
            self.live_buffer.pop(0)

        if len(self.live_buffer) < self.SEGMENT_SIZE:
            return {"status": "ACCUMULATING",
                    "have":   len(self.live_buffer),
                    "need":   self.SEGMENT_SIZE}

        segment  = self.live_buffer[-self.SEGMENT_SIZE:]
        combined = b"".join(s[1] for s in segment)
        live_sha = self._sha(combined)

        # Check if any DB segment SHA matches
        video_name = segment[0][2]
        start_sec  = segment[0][0]
        video_shas = self.frame_db.get(video_name, {})

        for db_sec_str, db_sha in video_shas.items():
            db_sec = float(db_sec_str)
            if abs(db_sec - start_sec) <= self.SEGMENT_SIZE + 2:
                if live_sha == db_sha:
                    return {"status":   "SEGMENT_SHA_VERIFIED",
                            "video":    video_name,
                            "at_sec":   start_sec,
                            "sha":      live_sha[:16] + "...",
                            "certainty":"HIGH — normalized segment SHA matched"}

        return {"status": "SEGMENT_SHA_MISMATCH",
                "note":   "Segment SHA differs — heavy re-encoding likely"}
...
def compute_confidence(results: dict) -> dict:
    """
    Aggregates all 7 verification results into a single score and verdict.
    ...
    if results.get("timestamp") == "VERIFIED":
        score += 5
        passed.append("✓ Timestamp progression natural")
    else:
        failed.append("✗ Timestamp progression irregular")

    # ── TIER 3 — Supporting hint (5 pts) ───────────────────
    sha_status = results.get("norm_sha", "")
    if sha_status in ("SHA_VERIFIED", "SEGMENT_SHA_VERIFIED"):
        score += 5
        passed.append("✓ Normalized SHA matched")
    else:
        failed.append("✗ Normalized SHA not matched (may be re-encoding)")
...
def preprocess_worker(frame_q: Queue, gray_q: Queue, stop: Event, cfg: Config):
    ...
    while not stop.is_set():
        ...
        # Pass both color crop (for zone/sha verifiers) and gray (for phash)
        if not gray_q.full():
...
def match_worker(hash_q: Queue, result_q: Queue, stop: Event, cfg: Config):
    ...
    db.load()
    db.build_index()
    logging.info("[MATCH] ready — %d entries", len(db.hashes))

    # ── Load verification databases ──────────────────────────
    vdir = Path(cfg.verify_db_dir)

    scene_verifier = None
    scene_db_path  = str(vdir / "scene_cuts.json")
    if Path(scene_db_path).exists():
        scene_verifier = SceneCutTimeline(scene_db_path)
        scene_verifier.load()
    else:
        logging.warning("[MATCH] Scene cut DB not found: %s", scene_db_path)

    motion_verifier = None
    motion_db_path  = str(vdir / "motion.json")
    if Path(motion_db_path).exists():
        motion_verifier = MotionPatternVerifier(motion_db_path)
        motion_verifier.load()
    else:
        logging.warning("[MATCH] Motion DB not found: %s", motion_db_path)

    zone_verifier  = None
    zone_db_path   = str(vdir / "zones.json")
    if Path(zone_db_path).exists():
        zone_verifier = SpatialZoneVerifier(zone_db_path)
        zone_verifier.load()
    else:
        logging.warning("[MATCH] Zone DB not found: %s", zone_db_path)

    sha_verifier  = None
    sha_db_path   = str(vdir / "norm_sha.json")
    if Path(sha_db_path).exists():
        sha_verifier = NormalizedSHAVerifier(sha_db_path)
        sha_verifier.load()
    else:
        logging.warning("[MATCH] SHA DB not found: %s", sha_db_path)

    # Always-on verifiers (no pre-built DB needed)
    seq_verifier   = SequenceVerifier(sequence_len=10, max_gap_sec=2.0)
    cycle_verifier = CycleAwareVerifier(tolerance_sec=3.0, min_chain=5)
    ...
    while not stop.is_set():
        ...
        # Feed motion verifier regardless of match
        if motion_verifier:
            motion_verifier.add_live_frame(gray)

        # Feed zone verifier regardless of match
        if zone_verifier:
        ...
        # Step 6 — Cycle-aware timestamp
        if video_sec is not None:
            cyc_result = cycle_verifier.add_match(video_name, video_sec, ts)
            verify_results["timestamp"] = cyc_result.get("status", "UNKNOWN")
        else:
            verify_results["timestamp"] = "NO_TIMESTAMP"

        # Step 7 — Normalized SHA (hint)
        if sha_verifier and video_sec is not None:
            sha_result = sha_verifier.verify_segment(crop, video_name, video_sec)
            verify_results["norm_sha"] = sha_result.get("status", "UNKNOWN")
        else:
            verify_results["norm_sha"] = "NO_DB"
...
def build_verify_dbs(video_dir: str, verify_db_dir: str, sample_fps: int = 5):
    ...
    Creates in verify_db_dir:
        ...
        norm_sha.json     — normalized SHA per frame per video
    """
    ...
    print("\n  [1/4] Scene cut timing patterns ...")
    sc = SceneCutTimeline(str(Path(verify_db_dir) / "scene_cuts.json"))
    sc.build(video_dir, sample_fps=sample_fps)

    print("\n  [2/4] Motion energy profiles ...")
    mp_ = MotionPatternVerifier(str(Path(verify_db_dir) / "motion.json"))
    mp_.build(video_dir)

    print("\n  [3/4] Spatial zone brightness profiles ...")
    sz = SpatialZoneVerifier(str(Path(verify_db_dir) / "zones.json"))
    sz.build(video_dir)

    print("\n  [4/4] Normalized SHA-256 per frame ...")
    ns = NormalizedSHAVerifier(str(Path(verify_db_dir) / "norm_sha.json"))
    ns.build(video_dir, sample_fps=1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Live Frame Matcher v4 — 7-step verification pipeline",
...
Examples:
  ...
  # Step 3 — Build all verification databases (scene cuts, motion, zones, SHA)
  python live_frame_matcher_optimized_4.py build-verify-db --video-dir C:\videos
  ...
  python live_frame_matcher_optimized_4.py test-match --db C:\hash_db\phash_database.pkl
    ...
    p_verify = sub.add_parser("build-verify-db",
                               help="Build scene-cut, motion, zone, SHA databases")
    ...
    if args.cmd == "build-db":
        ...
        print("  TIER 2 (raises confidence): Motion + Zones + Timestamp")
        print("  TIER 3 (hint): Normalized SHA-256")
        print("=" * 66 + "\n")
