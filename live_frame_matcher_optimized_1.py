"""
Live Video Frame Matcher — Windows 12-Core Pipeline (OPTIMIZED)
================================================================
...
All stages run simultaneously via task-parallelism on 12 cores.

OPTIMIZATIONS APPLIED (vs original):
  1. hamming_distance()  — single int XOR + popcount instead of byte-by-byte loop
                           (~10-50x faster per call)
  2. compute_phash_bytes() — np.packbits() instead of nested Python loop
                           (~20x faster bit packing)
  3. VP-tree build        — uses fast hamming + numpy-vectorized median
                           (tree build: seconds → milliseconds)
  4. VP-tree build        — iterative with explicit stack (no recursion limit)
  5. Precomputed int cache — hashes stored as Python ints for instant XOR
  6. Logo inpainting      — conditional skip when no logo detected recently
  7. Import JSON hashes   — can import from video_preprocess.py JSON files
                           directly (no re-encoding needed)

Requirements (install once):
    pip install opencv-python-headless numpy imagehash pillow scipy mss pygetwindow

Architecture:
    ┌─────────────┐    ┌──────────────┐    ┌──────────────┐    ┌─────────────┐
    │  CAPTURE     │───▶│  PREPROCESS  │───▶│  PHASH       │───▶│  MATCH      │
    │  (1 core)    │    │  crop+draw   │    │  (2 cores)   │    │  VP-tree    │
    │  mss grab    │    │  (1 core)    │    │  compute     │    │  O(log n)   │
    └─────────────┘    └──────────────┘    └──────────────┘    └─────────────┘
         ▲                                                          │
         │                    RESULT QUEUE  ◀───────────────────────┘
         └──────────────── continuous loop ─────────────────────────┘
...
class Config:
    ...
    # --- Source window ---
    window_title: str = "SABONG"          # partial match on browser window title
...
def _build_vptree(points, point_ints, labels):
    ...
    # Stack entries: (points, point_ints, labels, parent, is_left)
    # We build the tree bottom-up by post-processing
    root = None
    stack = [(list(range(len(points))), None, None)]  # indices, parent, is_left

    # Use a work stack to avoid recursion
    # Each item: (indices_list, parent_node_ref, is_left_child)
    import collections
    WorkItem = collections.namedtuple('WorkItem', ['indices', 'parent', 'is_left'])

    # Fully iterative build using explicit stack — no recursion limit issues
    # Stack items: (indices, parent_node, is_left_child)
    # We use a two-pass approach: first create nodes, then link them

    root = None
    # Each stack entry: (idxs, callback)
    # callback is a function that sets the node on its parent
    stack = []

    # We'll use a work list approach:
    # Push work items, process them, assign children via closures
    import collections
...
class HashDB:
    ...
    OPTIMIZATION: Can import hashes directly from video_preprocess.py JSON
    files without re-encoding videos.
    """
    ...
    def build_index(self):
        """Build VP-tree (call after load / add)."""
        if self.hashes:
    ...
    def query(self, target: bytes):
        ...
        # Fallback: brute force (still optimized with int comparison)
        target_int = bytes_to_int(target)
...
def main():
    parser = argparse.ArgumentParser(
        description="Live frame matcher — capture, pHash, match on 12 cores (OPTIMIZED)")
    sub = parser.add_subparsers(dest="cmd")

    # --- run ---
    p_run = sub.add_parser("run", help="Start the live matching pipeline")
    p_run.add_argument("--window", default="SABONG",
                       help="Partial browser window title to capture")
...
def _run_benchmark(db_path: str, n_queries: int = 100):
    """Benchmark the optimized VP-tree build and query performance."""
    print("\n" + "=" * 60)
    print("  BENCHMARK — Optimized Live Frame Matcher")
    print("=" * 60)
    ...
    print(f"  VP-tree build: {build_ms:.1f} ms")

    # Run queries
    print(f"\n  Running {n_queries} random queries...")
    query_times = []
    matches = 0

    for i in range(n_queries):
        # Use a random hash from the DB as query (guarantees at least one match)
    ...
    print(f"\n  Brute-force comparison (same {n_queries} queries)...")
    db_brute = HashDB(db_path, threshold=12)
    db_brute.load()
    # Don't build index — forces brute force fallback

    brute_times = []
    for i in range(min(n_queries, 10)):  # only 10 for brute force (it's slow)
    ...
    print(f"    Brute mean ({len(brute_times)} queries): {brute_times.mean():.2f} ms")
    speedup = brute_times.mean() / query_times.mean() if query_times.mean() > 0 else 0
    print(f"    VP-tree speedup: {speedup:.0f}x faster")
