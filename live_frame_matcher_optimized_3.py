...
RESOLUTION CHANGE vs original:
    Original was calibrated for 3440×1440 ultrawide.
    This build targets 1920×784 (source_w=1920, source_h=784).
    All crop/mask coordinates are recalibrated accordingly.
    proc_w/proc_h kept at 1907×720 (cropped arena) for DB compatibility.
...
OPTIMIZATIONS APPLIED (v2 vs v1):
  ...
  8. [NEW] VP-tree cache   — save/load built VP-tree to skip rebuild
  9. [NEW] Core allocation — tuned for 24-core i9 285K
 10. [NEW] Duplicate import fix — removed dead code in _build_vptree
 11. [NEW] 1920×784 support — all crop/mask coords recalibrated
...
# ---------------------------------------------------------------------------
# Configuration — recalibrated for 1920×784 browser (fullscreen)
#
# Source resolution: 1920×784
#
# Arena crop (in source space):
#   x=5, y=2, w=1907, h=720
#   → proc_w=1907, proc_h=720  (or keep as-is if DB was built at 1907×720)
#
# Drawbox masks (POST-CROP coordinates, relative to the 1907×720 cropped frame).
#   Masks are applied AFTER the crop step in preprocess_worker — no crop-offset
#   adjustment is needed. Use the same values as --mask in video_preprocess.py.
#   mask 1 (watermark):     x=650, y=286, w=495, h=191
#   mask 2 (bottom-right):  x=1178, y=520, w=82, h=61
#
# FIX: original Config used 3440×1440 source coords for everything.
#      Now source_w/h = 1920×784, focus region and masks are recalibrated.
# ---------------------------------------------------------------------------
@dataclass
class Config:
    ...
    # FIX: Capture resolution updated to 1920×784
    source_w: int = 1920
    source_h: int = 784

    # Internal processing resolution — arena after crop.
    # Matches the crop dimensions: 1907×720.
    # Keep consistent with video_preprocess.py so hash DBs are compatible.
    proc_w: int = 1907
    proc_h: int = 720

    # --- Focus / crop region in 1920×784 source space ---
    # FIX: recalibrated from 3440×1440 values to 1920×784
    # Crop: x=5, y=2, w=1907, h=720
    # Boundary: (5+1907)=1912 <=1920 OK | (2+720)=722 <=784 OK
    focus_x: int = 5
    focus_y: int = 2
    focus_w: int = 1907
    focus_h: int = 720

    # --- Drawbox masks in POST-CROP (1907×720) coordinates ---
    # Primary mask (center watermark + timestamp): x=650, y=286, w=495, h=191
    # FIX: original used a single mask_x/y/w/h. Now supports two masks via
    # mask_regions list. Kept scalar fields for backwards compatibility with
    # single-mask callers, set to the primary watermark mask.
    mask_x: int = 650
    mask_y: int = 286
    mask_w: int = 495
    mask_h: int = 191
    ...
    # Secondary mask (bottom-right logo): x=1178, y=520, w=82, h=61
    # FIX (new field): second static mask region, applied alongside mask_x/y/w/h
    mask2_x: int = 1178
    mask2_y: int = 520
    mask2_w: int = 82
    mask2_h: int = 61
    ...
    # FIX: reduced from 0.08 to 0.05 for 1907×720 frame (same reason as
    # video_preprocess.py — prevents large false-positive blobs)
...
def hamming_distance_int(a_int: int, b_int: int, n_bytes: int = 32) -> int:
    xor = a_int ^ b_int
    # FIX: if xor requires more than n_bytes to represent (shouldn't happen
    # with valid hashes, but can occur if hashes of different sizes are mixed),
    # to_bytes raises OverflowError. Guard with byte_length from bit_length.
...
def _build_vptree(points, point_ints, labels):
    ...
    while stack:
        ...
        # FIX: when all distances equal the median, every element goes to
        # right_idx and left_idx is empty, causing infinite stack growth
        # (the vp node is never consumed). Detect this degenerate split and
        # force a 50/50 partition to avoid unbounded recursion depth.
...
def _search_vptree(node, target_int, threshold, best, n_bytes=32):
    ...
    while stack:
        ...
        # FIX: original pruning logic had the branch conditions inverted.
        # Correct VP-tree search:
        #   if d - threshold <= current.threshold: search left (inner ball)
        #   if d + threshold >= current.threshold: search right (outer ball)
        # The original code used best[0] (current best distance) instead of
        # the query threshold for pruning — this is incorrect and causes
        # valid matches to be skipped. Fixed below.
...
class HashDB:
    ...
    def load(self):
        ...
        if self.hashes:
            # FIX: original hardcoded reshape width to 32. If hashes are
            # actually 32 bytes (phash_size=16 → 256 bits → 32 bytes) this
            # is fine. But if a DB was built with a different hash_size it
            # would silently produce a wrong array shape. Use the actual
            # hash length from the first entry.
            ...
            logging.info("Numpy hash array ready (%d × %d bytes)", len(self.hashes), hash_len)
    ...
    def build_index(self):
        ...
        try:
            # FIX: setrecursionlimit was set AFTER building the VP-tree,
            # making it useless for protecting the build itself. The iterative
            # build doesn't need this, but pickling a deep tree can hit the
            # default limit. Kept the guard here for pickling safety.
    ...
    def query(self, target: bytes):
        ...
        if self._hash_array is not None:
            ...
            # FIX: broadcasting requires target_bytes shape to match the
            # column count of _hash_array. If hash lengths differ (e.g. DB
            # built with different hash_size than current query), this will
            # raise a shape mismatch. Added guard.
            if target_bytes.shape[0] != self._hash_array.shape[1]:
                logging.warning(
                    "Hash length mismatch: query=%d bytes, DB=%d bytes. "
                    "Falling back to Python loop.",
                    target_bytes.shape[0], self._hash_array.shape[1])
            else:
    ...
    def import_json(self, json_path: str):
        ...
        for frame in frames:
            ...
            # FIX: hex_hash from video_preprocess.py is a hex string from
            # imagehash.phash (e.g. "a1b2c3..."). bytes.fromhex() works
            # correctly here. However, compute_phash_bytes() in this file
            # produces raw bytes via np.packbits. Both produce 32-byte hashes
            # for hash_size=16. They are NOT bit-for-bit identical because
            # imagehash uses a different bit-ordering than np.packbits with
            # default big-endian. This is a known cross-compatibility issue —
            # if you build the DB with video_preprocess.py (imagehash) and
            # match with this script (compute_phash_bytes), distances will
            # be higher than expected. For best results, either rebuild the
            # DB using build-db mode of this script, or use imagehash in
            # both pipelines. Added a log warning here.
            try:
                h_bytes = bytes.fromhex(hex_hash)
            except ValueError as e:
                logging.warning("Skipping frame t=%s: invalid hex hash (%s)", timestamp, e)
                continue

            # Validate expected hash length (32 bytes for phash_size=16)
            if len(h_bytes) != 32:
                logging.warning(
                    "Unexpected hash length %d bytes at t=%s (expected 32). "
                    "Was video_preprocess.py run with hash_size=16?",
                    len(h_bytes), timestamp)
...
class MovingLogoDetector:
    ...
    def _detect_bbox(self, frame: np.ndarray):
        ...
        if best:
            ...
            # FIX: clamp uses padded bx/by (same fix as video_preprocess.py)
    ...
    def process(self, frame: np.ndarray) -> np.ndarray:
        ...
        if bbox is None:
            return frame
        x, y, w, h = bbox
        # FIX: guard against zero-area bbox before inpainting
        if w <= 0 or h <= 0:
            return frame
...
def _capture_win32(frame_q: Queue, stop: Event, cfg: Config, interval: float):
    ...
    while not stop.is_set():
        ...
        try:
            ...
            # Resize to source resolution (1920×784) if window reported different size
            fh, fw = frame.shape[:2]
...
# ---- Stage 2: Preprocess (crop + drawbox + resize to proc resolution) ----
def preprocess_worker(frame_q: Queue, gray_q: Queue, stop: Event, cfg: Config):
    """
    Crop to focus area (1920×784 source coords), apply drawbox masks,
    detect/remove floating logo, then:
      1. Resize to proc_w × proc_h if needed for DB compatibility.
      2. Convert to grayscale for pHash.
    """
    logging.info("[PREPROCESS] starting (capture: %dx%d → proc: %dx%d)",
    ...
    while not stop.is_set():
        ...
        # 1) Crop to focus region (source-space coordinates)
        crop = frame[cfg.focus_y : cfg.focus_y + cfg.focus_h,
                      cfg.focus_x : cfg.focus_x + cfg.focus_w]

        # FIX: guard against empty crop (e.g. focus region out of bounds)
        if crop.size == 0:
            logging.warning("[PREPROCESS] crop produced empty frame — skipping")
            continue

        # 2) Drawbox — black out primary static watermark (post-crop coords)
        # Clamp to actual crop dimensions before drawing
        ...
        # FIX (new): apply second static mask if enabled
...
def phash_pool_worker(gray_q: Queue, hash_q: Queue, stop: Event, cfg: Config):
    ...
    while not stop.is_set():
        ...
        if batch:
            ...
            for fut in futures:
                # FIX: fut.result() can raise if _phash_one throws. Wrap in
                # try/except so one bad frame does not kill the phash worker.
                try:
                    ts, h = fut.result()
                    if not hash_q.full():
                        hash_q.put((ts, h))
                except Exception as e:
                    logging.warning("[PHASH] frame hash failed: %s", e)
            batch.clear()
...
def result_consumer(result_q: Queue, stop: Event, cfg: Config):
    ...
    with open(log_path, "a", encoding="utf-8") as fout:
        while not stop.is_set():
            ...
            print("├─────────────────────────────────────────────────┤")
            # FIX: f-string column alignment uses len() which is accurate for
            # ASCII strings but Khmer Unicode characters are multi-byte and
            # the visual width of each glyph may differ in different terminals.
            # Column widths are cosmetic here and intentionally left as-is
            # since fixing requires platform-specific wcwidth() calls.
...
def main():
    parser = argparse.ArgumentParser(
        description="Live frame matcher — 1920×784 capture, pHash, match on 24 cores (OPTIMIZED v2)")
...
def _run_benchmark(db_path: str, n_queries: int = 100):
    ...
    print(f"\n  [3/3] Old Python brute-force (10 queries)...")
    db_brute = HashDB(db_path, threshold=12)
    db_brute.load()
    db_brute._hash_array = None
    db_brute.vptree = None
    brute_times = []
    ...
    print("=" * 60)
    print(f"  Python brute-force:  {brute_times.mean():>10.2f} ms")
    print(f"  Numpy vectorized:    {np_times.mean():>10.2f} ms")
    print(f"  VP-tree:             {query_times.mean():>10.2f} ms")
