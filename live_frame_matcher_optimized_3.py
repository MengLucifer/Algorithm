...
Tuned for:
    ...
    OS:   Windows 10 Pro
    MON:  3440×1440 ultrawide (browser fullscreen)
...
OPTIMIZATIONS APPLIED (v2 vs v1):
 ...
 11. [NEW] 3440×1440 support — all crop/mask coords scaled for ultrawide
...
# ---------------------------------------------------------------------------
# Configuration — scaled for 3440×1440 ultrawide browser (fullscreen)
#
# Original values were calibrated for 1920×1040.
# Scale factors:
#   scale_x = 3440 / 1920 = 1.7917
#   scale_y = 1440 / 1040 = 1.3846
#
# NOTE: The pipeline still outputs 1920×1040 frames to the pHash stage
# so that existing hash databases (built at 1920×1040) remain compatible.
# The capture stage grabs at 3440×1440, then resizes to source_w × source_h.
# All crop / mask coordinates below are in 3440×1440 space.
# ---------------------------------------------------------------------------
@dataclass
class Config:
    ...
    # Capture resolution — matches your 3440×1440 ultrawide browser
    source_w: int = 3440
    source_h: int = 1440

    # Internal processing resolution — keep at 1920×1040 so existing
    # hash databases (built from 1920×1040 recordings) stay compatible.
    # Captured frames are resized from source_w/h → proc_w/h before pHash.
    proc_w: int = 1920
    proc_h: int = 1040

    # --- Focus / crop region in 3440×1440 space ---
    # Original (1920×1040): x=5, y=3, w=1907, h=955
    ...
    # --- Drawbox mask in 3440×1440 space ---
    # Original (1920×1040): x=650, y=380, w=495, h=254
    # Scaled ×1.7917 / ×1.3846
    mask_x: int = 1165
    mask_y: int = 526
    mask_w: int = 887
...
def _capture_win32(frame_q: Queue, stop: Event, cfg: Config, interval: float):
    ...
    while not stop.is_set():
        ...
        try:
            ...
            # Resize to source resolution (3440×1440) if window reported different size
            fh, fw = frame.shape[:2]
...
def preprocess_worker(frame_q: Queue, gray_q: Queue, stop: Event, cfg: Config):
    """
    Crop to focus area (3440×1440 coords), apply drawbox mask,
    detect/remove floating logo, then:
      1. Resize from source_w/h (3440×1440) → proc_w/h (1920×1040)
         so the pHash matches your existing 1920×1040 hash database.
    ...
    while not stop.is_set():
        ...
        # 1) Crop to focus region (3440×1440 coordinates)
        crop = frame[cfg.focus_y : cfg.focus_y + cfg.focus_h,
                      cfg.focus_x : cfg.focus_x + cfg.focus_w]

        # 2) Drawbox — black out static watermark (3440×1440 coordinates)
        cv2.rectangle(
            crop,
        ...
        # 4) Resize to processing resolution (1920×1040) for DB compatibility
...
def main():
    parser = argparse.ArgumentParser(
        description="Live frame matcher — 3440×1440 ultrawide capture, pHash, match on 24 cores (OPTIMIZED v2)")
